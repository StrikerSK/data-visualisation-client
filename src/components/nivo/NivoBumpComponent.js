...
import {barDataGetter, lineDataGetter} from "../../lib/DataFetcher";
import {connect} from "react-redux";

const NivoBumpComponent = ({months, person, validity, sellType, color}) => {
	const [data, setData] = useState([]);
	const [isLoaded, changeLoadedState] = useState(false);

	const test = (result) => {
		const testVar = result.map(item => {
			delete item.month;
			const objArray = Object.values(item);
	...
	useEffect(() => {
		barDataGetter([months, person, validity, sellType], finaliseTransaction);
...
export default connect(mapStateToProps)(NivoBumpComponent);
